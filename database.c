...
retvalue table_newduplicatecursor(struct table *table, const char *key, struct cursor **cursor_p, const char **key_p, const char **data_p, size_t *datalen_p) {
	struct cursor *cursor;
	int dbret;
	DBT Key, Data;
	retvalue r;
...
"This should normally not happen (to avoid triggering bugs in the underlying BerkeleyDB)\n",
	...
	if (RET_WAS_ERROR(r))
		return r;
	r = database_table("packagenames.db", identifier,
			dbt_BTREEVERSIONS, readonly?DB_RDONLY:DB_CREATE, &secondary_table);
	assert (r != RET_NOTHING);
	if (RET_WAS_ERROR(r))
		return r;

	if (table->berkeleydb != NULL && secondary_table->berkeleydb != NULL) {
